...
void ejecutar_fopen(char* nombreArchivo){
    //log_debug(debug_logger,"Envio este nombre de archivo: %s", nombreArchivo);
...
void ejecutarFtruncate(char* nombreArchivo, uint32_t tamanioNuevo){
    ...
    enviarEnteroYString(size,nombreArchivo,fd_kernel,trace_logger,ESCRITURA_ARCHIVO_EXITOSA);
    free(nombreArchivo);
}
...
void ejecutar_finalizarEscrituraArchivo(char* nombreArchivo, uint32_t puntero, uint32_t tamanio, uint32_t direccionFisica, void* datos){
    ...
    enviarEnteroYString(size,nombreArchivo,fd_kernel,info_logger,ESCRITURA_ARCHIVO_EXITOSA);
    free(datos);
...
void ejecutar_finalizarLecturaArchivo(char* nombreArchivo){
    ...
    enviarEnteroYString(size,nombreArchivo,fd_kernel,info_logger,LECTURA_ARCHIVO_EXITOSA);
}
